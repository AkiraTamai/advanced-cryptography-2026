        ...
        return self.reduce(
            sum(
                coefficient * math.prod(
                    value ** exponent for value, exponent in zip(point, exponents)
                )
...
class UnivariatePolynomial(Polynomial):
    ...
    def evaluate(self, x: int) -> int:
        ...
        """
        return self.reduce(
            sum(c * x**i for i, c in enumerate(self.coefficients))
        )
...
class SumCheck:
    ...
    def construct_round_polynomial(self, challenges: list[int]) -> UnivariatePolynomial:
        ...
        """
        i = len(challenges)                                       # 今の変数の位置
        degree = max(exponents[i] for exponents in self.f.terms)  # 箱の数 − 1
        coefficients = [0] * (degree + 1)

        for tail in self.gen_boolean_points(self.n - i - 1):       # 後ろ: 0/1の全組み合わせ
            for exponents, coefficient in self.f.terms.items():
                value = coefficient
                for j, r in enumerate(challenges):                 # 前: challengeを代入
                    value *= r ** exponents[j]
                for j, b in enumerate(tail):                       # 後ろ: 0 か 1を代入
                    value *= b ** exponents[i + 1 + j]
                slot = exponents[i]                                # 今: 箱の番号
                coefficients[slot] = self.f.reduce(coefficients[slot] + value)
    ...
    def prove(self, challenges:list[int] | None = None) -> list[tuple[UnivariatePolynomial, int]]:
        ...
        """
        proof = []
        fixed = []                       # ここまでに決まった challenge

        for i in range(self.n):          # 変数の数だけラウンドがある
            g = self.construct_round_polynomial(fixed)
            if challenges is not None:
                r = challenges[i]
            else:
                r = random.randrange(self.p)
            proof.append((g, r))
            fixed.append(r)              # 次のラウンドでは代入される側に回る
    ...
    def verify(
        ...
        """
        expected = self.f.reduce(claimed_sum)   # いま検査したい主張
        fixed = []                              # 受け取った challenge

        for g, r in proof:
            if self.f.reduce(g.evaluate(0) + g.evaluate(1)) != expected:
                return False                    # 検査 A に落ちた
            expected = g.evaluate(r)            # 次の主張に更新
            fixed.append(r)

        return expected == self.f.evaluate(tuple(fixed))   # 検査 B
