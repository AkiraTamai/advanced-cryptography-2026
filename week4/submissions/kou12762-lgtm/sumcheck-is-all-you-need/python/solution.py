...
class UnivariatePolynomial(Polynomial):
    ...
    def evaluate(self, x: int) -> int:
        ...
        for i,coefficient in enumerate(self.coefficients):
            result += coefficient * pow(x,i,self.p)
        return self.reduce(result)
...
class SumCheck:
    ...
    def construct_round_polynomial(self, challenges: list[int]) -> UnivariatePolynomial:
        ...
        """
        i = len(challenges)                      # 自由変数 t の添字
        num_free = self.n - i - 1                # 総和を取る変数の個数

        max_degree = max(exponents[i] for exponents in self.f.terms)
        coefficients = [0] * (max_degree + 1)

        for suffix in self.gen_boolean_points(num_free):
            assignment = list(challenges) + [None] + list(suffix)
            for exponents, coefficient in self.f.terms.items():
                value = coefficient
                for j, exponent in enumerate(exponents):
                    base = assignment[j]
                    if base is None:
                        continue                 # tの分は掛けずに次数として使う
                    value *= pow(base, exponent, self.p)
                coefficients[exponents[i]] += value

        return UnivariatePolynomial(
            [self.f.reduce(c) for c in coefficients],
            self.p
        )
    ...
    def prove(self, challenges:list[int] | None = None) -> list[tuple[UnivariatePolynomial, int]]:
        ...
        for i in range(self.n):
            ...
            r = random.randrange(self.p) if challenges is None else challenges[i]
            proof.append((g, r))
            fixed.append(r)
