...
class Polynomial:
    ...
    def evaluate(self, point: tuple[int, ...]) -> int:
        ...
        for exponents, coefficient in self.terms.items():
            ...
            for base, exponent in zip(point, exponents):
                term_value *= base ** exponent
            total += term_value
...
class SumCheck:
    ...
    def construct_round_polynomial(self, challenges: list[int]) -> UnivariatePolynomial:
        ...
        """
        idx = len(challenges)
        max_degree = max(exponents[idx] for exponents in self.f.terms)
        ...
        for exponents, coefficient in self.f.terms.items():
            ...
            for j in range(idx + 1, self.n):
                if exponents[j] == 0:
                    factor *= 2
            coefficients[exponents[idx]] += factor
    ...
    def verify(
        ...
        for round_polynomial, r_i in proof:
            round_sum = self.f.reduce(round_polynomial.evaluate(0) + round_polynomial.evaluate(1))
            if round_sum != expected:
                return False
            expected = self.f.reduce(round_polynomial.evaluate(r_i))
            challenges.append(r_i)

        return expected == self.f.reduce(self.f.evaluate(tuple(challenges)))
