...
class Polynomial:
    ...
    def evaluate(self, point: tuple[int, ...]) -> int:
        ...
        """
        result = 0
        for t, a in self.terms.items():
            mul = a
            for i in range(len(point)):
                mul *= point[i]**t[i]

            result += self.reduce(mul)
...
class UnivariatePolynomial(Polynomial):
    ...
    def evaluate(self, x: int) -> int:
        ...
        for i in range(len(self.coefficients)):
            result += (self.coefficients[i] * x**i) % self.p
...
class SumCheck:
    ...
    def construct_round_polynomial(self, challenges: list[int]) -> UnivariatePolynomial:
        ...
        max_degree = max(term[r] for term in self.f.terms)
        coeffs = [0] * (max_degree + 1)

        for y_tuple in self.gen_boolean_points(num_free):
            for term, coeff in self.f.terms.items():
                value = coeff

                for j in range(r):
                    value *= challenges[j]**term[j]

                for k in range(num_free):
                    value *= y_tuple[k]**term[r + 1 + k]

                t_degree = term[r]
                coeffs[t_degree] += value

        coeffs = [c % self.p for c in coeffs]

        return UnivariatePolynomial(coeffs, self.p)
    ...
    def prove(self, challenges:list[int] | None = None) -> list[tuple[UnivariatePolynomial, int]]:
        ...
        for n in range(self.n):
            g_i = self.construct_round_polynomial(fixed)
    ...
    def verify(
        ...
        for i, (g_i, r_i) in enumerate(proof):
            check_value = (g_i.evaluate(0) + g_i.evaluate(1)) % self.p

            if i == 0:
                target = claimed_sum % self.p
            else:
                target = g.evaluate(r) % self.p
        ...
        final_check = g.evaluate(r) % self.p
        expected = self.f.evaluate(tuple(fixed)) % self.p

        return final_check == expected
