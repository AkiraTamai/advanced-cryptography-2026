...
def share(secret: int, randomness: list[int], modulus: int) -> ShareVector:
    ...
    head = [value % modulus for value in randomness]
    tail = (secret - sum(head)) % modulus
    return head + [tail]
...
def beaver_multiply(
    ...
    _validate_modulus(modulus)
    if len(triple) != 3:
        raise ValueError("triple must contain three share vectors")
...
def ot_receiver_request(
    ...
    if choice == 1:
        request = (request * sender_public) % OT_P
    return request
...
def ot_sender_encrypt(
    ...
    return (
        xor_bytes(message_0, derive_pad(shared_0, 0, len(message_0))),
        xor_bytes(message_1, derive_pad(shared_1, 1, len(message_1))),
    )
...
def ot_receiver_decrypt(
    ...
    return xor_bytes(selected, derive_pad(shared, choice, len(selected)))
...
def gmw_and(
    ...
    # P1 learns r01 XOR (x0 AND y1), P0 learns r10 XOR (x1 AND y0).
    ...
    return (
        (x0 & y0) ^ mask_01 ^ received_10,
        (x1 & y1) ^ mask_10 ^ received_01,
    )
