...
def share(secret: int, randomness: list[int], modulus: int) -> ShareVector:
    ...
    share = []
    for r in randomness:
        total = (total + r) % modulus
        share.append(r % modulus)
    result = (secret - total) % modulus
    return share + [result]
...
def add_shares(
    ...
    result = []
    for (left_share, right_share) in zip(left_shares,right_shares):
        result.append((left_share + right_share) % modulus)
    return result
...
def beaver_multiply(
    ...
    de = (d * e) % modulus
    results = []
    for i, (a_i, b_i, c_i) in enumerate(zip(triple[0], triple[1], triple[2])):
        result = (c_i + d * b_i + e * a_i) % modulus
        if i == 0:
            result = (result + de) % modulus
...
def ot_receiver_request(
    ...
    """
    validate_group_element(sender_public)
    validate_choice(choice)
    validate_receiver_scalar(receiver_secret)
    if choice == 0:
...
def ot_sender_encrypt(
    ...
    """
    validate_sender_scalar(sender_secret)
    A = ot_sender_setup(sender_secret)
    B_over_A = request * pow(A,-1,OT_P) % OT_P
...
def ot_receiver_decrypt(
    ...
    validate_group_element(sender_public, "sender public")
    validate_choice(choice)
    validate_receiver_scalar(receiver_secret)

    target = ciphertexts[choice]
    shared = pow(sender_public, receiver_secret, OT_P)
    pad =derive_pad(shared, choice, len(target))
    return xor_bytes(target, pad)
...
def gmw_and(
    ...
    """   
    _validate_bit(y_shares[0], "message_0")
    _validate_bit(y_shares[1], "message_1")
    if len(ot_secrets) != 2:
        raise ValueError("ot_secrests needs two tuples")
    outputs = []
    for i in range(2):
        sender_secret = ot_secrets[i][0]
        receiver_secret = ot_secrets[i][1]
        choice = y_shares[1 - i]
        validate_choice(choice)
        message_0 = masks[i]
        message_1 = masks[i] ^ x_shares[i]
        output = _ot_transfer_bit(
            message_0,
            message_1,
            choice,
            sender_secret,
    ...
    results = []
    for i in range(2):    
        results.append((x_shares[i] & y_shares[i]) ^ (masks[i] ^ outputs[1-i]))
