...
def share(secret: int, randomness: list[int], modulus: int) -> ShareVector:
    ...
    if len(randomness) < 1:
        raise ValueError("randomness must contain at least one share")

    normalized_randomness = [value % modulus for value in randomness]
    secret_mod = secret % modulus
    final_share = (secret_mod - sum(normalized_randomness)) % modulus
    return [*normalized_randomness, final_share]
...
def reconstruct(shares: ShareVector, modulus: int) -> int:
    ...
    _validate_modulus(modulus)
    if len(shares) < 2:
        raise ValueError("shares must contain at least two parties")
...
def beaver_multiply(
    ...
    """
    _validate_modulus(modulus)
    _validate_same_share_count(x_shares, y_shares, triple[0], triple[1], triple[2])

    d = reconstruct(sub_shares(x_shares, triple[0], modulus), modulus)
    e = reconstruct(sub_shares(y_shares, triple[1], modulus), modulus)

    scaled_b = scale_shares(triple[1], d, modulus)
    scaled_a = scale_shares(triple[0], e, modulus)
    result = add_shares(add_shares(triple[2], scaled_b, modulus), scaled_a, modulus)
    result[0] = (result[0] + (d * e) % modulus) % modulus
    return result
...
def ot_sender_encrypt(
    ...
    branch0_shared = pow(request, sender_secret, OT_P)
    branch1_shared = pow((request * pow(sender_public := pow(OT_G, sender_secret, OT_P), -1, OT_P)) % OT_P, sender_secret, OT_P)

    pad0 = derive_pad(branch0_shared, 0, len(message_0))
    pad1 = derive_pad(branch1_shared, 1, len(message_1))
    return xor_bytes(message_0, pad0), xor_bytes(message_1, pad1)
...
def ot_receiver_decrypt(
    ...
    pad = derive_pad(shared, choice, len(ciphertexts[choice]))
    return xor_bytes(ciphertexts[choice], pad)
...
def gmw_and(
    ...
    if len(ot_secrets) != 2 or len(ot_secrets[0]) != 2 or len(ot_secrets[1]) != 2:
        raise ValueError("ot_secrets must contain two (sender_secret, receiver_secret) pairs")
    ...
    session_01_output = _ot_transfer_bit(
        r01,
        r01 ^ x0,
        y1,
        sender_secret_01,
    ...
    session_10_output = _ot_transfer_bit(
        r10,
        r10 ^ x1,
        y0,
        sender_secret_10,
    ...
    share0 = r01 ^ session_10_output ^ (x0 & y0)
    share1 = r10 ^ session_01_output ^ (x1 & y1)
